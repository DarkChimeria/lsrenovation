...
export class InvoiceComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.customerService.getAllCustomer().then(cars => this.customer = cars);
        this.values = [];
        this.values = [
            { Designation: 'Article 1', Quantity: 34.00, Unit: { Name: 'M2' }, Amount: 1, TotalAmount: 34 },
            { Designation: 'Article 2', Quantity: 34.00, Unit: { Name: 'M2' }, Amount: 1, TotalAmount: 34 },
            { Designation: 'Article 3', Quantity: 34.00, Unit: { Name: 'M2' }, Amount: 1, TotalAmount: 34 },
            { Designation: 'Article 4', Quantity: 34.00, Unit: { Name: 'M2' }, Amount: 1, TotalAmount: 34 }
        ];
    ...
    delete(item: Workline) {
        const index = this.values.indexOf(item);
        this.values.splice(index, 1);
    }
    onChangeCustomer(event) {
        if (event != null) { this.invoice.Customer = event.value; }
    }

    addWorkline(event = null) {
        this.workline.TotalAmount = 0;
        this.workline.TotalAmount += this.workline.Quantity * this.workline.Amount;
        this.totalAmount += this.workline.TotalAmount;
        this.workline.Unit = this.selectedUnit;
        this.values.push(this.workline);
    ...
    selectCar(event: Event, car: Invoice) {
        this.selectedInvoice = car;
    ...
    buildTableBody(data, columns) {
        ...
        data.forEach(function (row) {
            ...
            columns.forEach(function (column) {
                ...
                dataRow.push({ text: row[column].toString(), style: column === 'Designation' ? 'itemTitle' : column === 'TotalAmount' || column === 'Amount' || column === 'Quantity' ? 'itemTotal' : 'itemNumber' });
            }),
            ...
            content: [
                ...
                // Billing Details
                {
                    columns: [
                        ...
                        {
                            text: this.selectedCustomer.FirstName + ' ' + this.selectedCustomer.LastName,
                            style: 'invoiceBillingDetails'
                        },
                    ]
                },
                ...
                // Billing Address
                {
                    columns: [
                        ...
                        {
                            // tslint:disable-next-line:max-line-length
                            text: this.selectedCustomer.Address + '\n' + this.selectedCustomer.PostalCode + ' ' + this.selectedCustomer.City,
                            style: 'invoiceBillingAddress'
                        },
                    ]
                },
                ...
                this.table(this.values, ['Designation', 'Quantity', 'UnitToShow', 'Amount', 'TotalAmount']), // table
